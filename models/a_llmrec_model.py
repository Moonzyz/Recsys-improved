...
class A_llmrec_model(nn.Module):
    def __init__(self, args):
        ...
        # CF-RecSys (mô hình collaborative filtering ban đầu)
    ...
    def load_model(self, args, phase1_epoch=None):
    ...
    def pre_train_phase1(self, data, optimizer, batch_iter):
        ...
        #log_emb là biểu diễn cuối cùng cho chuỗi sản phẩm lịch sử người dùng
        #pos_emb là biểu diễn của sản phẩm positive, chính là sản phẩm tiếp theo trong training, neg là ngược lại
        log_emb_ = log_emb[indices]
        pos_emb_ = pos_emb[indices]
        neg_emb_ = neg_emb[indices]
        #lấy sản phẩm cuối cùng pos và neg của người dùng
    ...
    #     """
    #     Hàm predict: Dự đoán điểm tương tự (logits) giữa user embedding và item embedding.

    #     Args:
    #         user_ids: Danh sách ID người dùng (list hoặc numpy array).
    #         log_seqs: Chuỗi lịch sử các sản phẩm mà người dùng đã tương tác (numpy array).
    #         item_indices: Danh sách ID các sản phẩm để dự đoán (list hoặc numpy array).

    #     Returns:
    #         logits: Điểm dự đoán cho từng sản phẩm trong `item_indices` (torch.Tensor).
    #     """
    #     # Lấy user embedding từ chuỗi lịch sử tương tác
    #     print(f"Device of RecSys Model: {next(self.recsys.model.parameters()).device}")
    #     with torch.no_grad():
    #         log_feats = self.recsys.model.log2feats(torch.LongTensor(log_seqs.to('cpu')))

    #     # Lấy embedding của bước cuối cùng trong chuỗi lịch sử (last hidden state)
    #     final_feat = log_feats[:, -1, :]  # [batch_size, embedding_dim]

    #     # Lấy item embedding từ hàm `get_item_emb`
    #     item_embs = self.get_item_emb(item_indices)  # [num_items, embedding_dim]

    #     # Tính điểm dự đoán (logits): tích vô hướng giữa user embedding và item embedding
    #     logits = item_embs.matmul(final_feat.unsqueeze(-1)).squeeze(-1)  # [batch_size, num_items]

    #     return logits

    # def predict(self, user_ids, log_seqs, item_indices):
    # # Đồng bộ thiết bị giữa tensor và mô hình
    #     device = next(self.recsys.model.parameters()).device
    #     log_seqs = log_seqs.to(device)
    #     item_indices = item_indices.to(device)
        
    #     # Lấy embedding chuỗi lịch sử
    #     log_feats = self.recsys.model.log2feats(log_seqs.to('cpu'))
    #     final_feat = log_feats[:, -1, :]  # User embedding cuối cùng

    #     # Lấy embedding của các sản phẩm
    #     item_embs = self.get_item_emb(item_indices.to('cpu'))
    ...
    #     # Lấy embedding chuỗi lịch sử
    #     log_feats = self.recsys.model.log2feats(log_seqs.to('cpu'))
    #     final_feat = log_feats[:, -1, :]  # User embedding cuối cùng, kích thước [batch_size, hidden_dim]

    #     # Lấy embedding của các sản phẩm
    #     item_embs = self.get_item_emb(item_indices.to('cpu'))  # Kích thước [num_items, hidden_dim]

    #     # Tính toán điểm dự đoán
    #     # Đảm bảo final_feat được mở rộng để nhân ma trận
    #     logits = item_embs.matmul(final_feat.T)  # Kích thước: [num_items, batch_size]

    #     return logits.T  # Chuyển kết quả về [batch_size, num_items]
    ...
    def predict(self, user_ids, log_seqs, item_indices):
        ...
        # Lấy embedding của các sản phẩm
        item_embs = self.get_item_emb(item_indices.to('cpu'))  # [num_items, embedding_dim]
